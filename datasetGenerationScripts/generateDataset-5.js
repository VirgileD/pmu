...
var trainFile = "/home/virgile/projects/pmu/testFiles/train-5.csv";
...
function generateSet(result, goodPronos, cheval) {
  ...
  if(cheval> result.nbPartants||isNaN(result.refCote[cheval]))  {
    set = ";0";
    for(var gp =0; gp< goodPronos.length;gp++) {
      set += ";0";
    }
  } else {
    // ajout de la cote
            ...
            if(result.finish) {
              for(var i = 1; i <= result.nbPartants;i++) {
                ...
                if(result.refCote[i] && !isNaN(result.refCote[i])) {
                  ...
                  myS+=generateSet(result, goodPronos, i);
                  // on doit avoir toujours le meme nombre de features
                  // 20 est le nomre de partants max comme l'inque db.courses1.find().sort({ts:-1}).limit(1)
                  for(var j = 1; j <= 20;j++) {
                    if(i!=j) {
                      myS+=generateSet(result, goodPronos, j);
                    }
                  }
